...
object WikipediaRanking extends WikipediaRankingInterface {
  ...
  val conf: SparkConf = new SparkConf().setAppName("Wikipidia Programming Language Ranking").setMaster("local[*]")
   ...
   */
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.filter(_.mentionsLanguage(lang)).aggregate(0)((x, _) => x + 1, _ + _)
