...
trait Huffman extends HuffmanInterface:
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs match {
    case Nil => Nil
    case (char, weight) :: freqs1 => insertWeightOrder(new Leaf(char, weight), makeOrderedLeafList(freqs1))
  }

  private def insertWeightOrder(leaf: Leaf, leafs: List[Leaf]): List[Leaf] = leafs match {
    case Nil => List(leaf)
    case l :: leafs1 => 
      if(leaf.weight <= l.weight) leaf :: l :: leafs1
      else l :: insertWeightOrder(leaf, leafs1)
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def recur(tree2: CodeTree, bits: List[Bit]): List[Char] = tree2 match {
      case Leaf(c, _) if (bits.isEmpty) => List(c) 
      case Leaf(c, _) => c :: recur(tree, bits)
      case Fork(l, r, _, _) if bits.head == 0 => recur(l, bits.tail)
      case Fork(l, r, _, _) => recur(r, bits.tail)
    }
    recur(tree, bits)
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def recur(tree: CodeTree, char: Char): List[Bit] = tree match {
      case l: Leaf => Nil
      case f: Fork => 
        if chars(f.left).contains(char) then 0 :: recur(f.left, char)
        else 1 :: recur(f.right, char)
    }
    if text.isEmpty then Nil
    else recur(tree, text.head) ++ encode(tree)(text.tail)
  }
