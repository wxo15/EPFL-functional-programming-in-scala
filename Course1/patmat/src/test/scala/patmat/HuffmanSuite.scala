...
class HuffmanSuite extends munit.FunSuite:
  ...
  test("createNodeTree") {
    new TestTrees:
      assertEquals(createCodeTree("aaaaaabbccc".toList), Fork(Leaf('a',6),Fork(Leaf('b',2), Leaf('c',3), List('b','c'), 5), List('a','b','c'),11))
  }

  test("convert") {
    new TestTrees:
      assertEquals(convert(createCodeTree("aaaaaabbccc".toList)), List(('a', List(1)), ('b', List(0,1)), ('c', List(0,0)) ))
  }
