...
class KMeans extends KMeansInterface:
  ...
  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] =
    ...
    means.map(m => (m, if (pointsMeanMap.contains(m)) then ParSeq() ++ pointsMeanMap(m) else ParSeq())).toMap
  ...
  final def kMeans(points: ParSeq[Point], means: ParSeq[Point], eta: Double): ParSeq[Point] =
    ...
    if converged(eta, means, newMeans) then newMeans else kMeans(points, newMeans, eta) // your implementation need to be tail recursive
