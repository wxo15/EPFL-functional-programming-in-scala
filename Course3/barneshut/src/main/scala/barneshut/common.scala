...
extends Quad:
  ...
  val massX: Float = bodies.foldLeft(0f)({case (acc, body) => acc + (body.mass * body.x)/mass})
  val massY: Float = bodies.foldLeft(0f)({case (acc, body) => acc + (body.mass * body.y)/mass})
  ...
  def insert(b: Body): Quad = {
    if size > minimumSize then {
      val newFork = Fork(
          ...
          Empty(centerX - size/4, centerY + size/4, size/2),
          Empty(centerX + size/4, centerY + size/4, size/2))
      (bodies :+ b).foldLeft(newFork)({ case (fork, body) => fork.insert(body) })
    }
    else
  ...
  def updated(quad: Quad): Body =
    ...
    def traverse(quad: Quad): Unit = (quad: Quad) match
      ...
      case Leaf(_, _, _, bodies) =>
        ...
        bodies.map({ case (b: Body) =>addForce(b.mass, b.x, b.y) })
...
class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) extends SectorMatrixInterface:
  ...
  def clamp(x:Float) : Int = x.toInt max 0 min sectorPrecision

  def +=(b: Body): SectorMatrix =
    val sector: Int = clamp((b.y - boundaries.minY) / ((boundaries.maxY - boundaries.minY) / sectorPrecision) ) * sectorPrecision + 
                      clamp((b.x - boundaries.minX) / ((boundaries.maxX - boundaries.minX) / sectorPrecision) )
    matrix(sector) += b
    this
  ...
  def combine(that: SectorMatrix): SectorMatrix =
    matrix.zipWithIndex.foreach({ case(m, i) => m.combine(that.matrix(i)) })
    this
